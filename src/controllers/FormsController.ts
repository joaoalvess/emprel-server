...
class FormsController{
  async indexNotSend(request: Request, response: Response) {
    ...
    const users = await knex('users').whereNotIn('id', test).whereNot('email', 'inativo').orderBy('nome')
  ...
  async indexInapto(request: Request, response: Response) {
    ...
    switch (select) {
      case "data":
        const filterData = await knex('forms').where('data', String(selectDate)).where('apto', false)
        response.json(filterData)
      break
      case "infectado":
        const filterInfectado = await knex('forms').where('infectado', Boolean(infectado)).where('data', String(selectDate)).where('apto', false)
        response.json(filterInfectado)
      break
      case "contato_infectado":
        const filterContato = await knex('forms').where('contato_infectado', Boolean(contato_infectado)).where('data', String(selectDate)).where('apto', false)
        response.json(filterContato)
      break
      case "tosse":
        const filterTosse = await knex('forms').where('tosse', Boolean(tosse)).where('data', String(selectDate)).where('apto', false)
        response.json(filterTosse)
      break
      case "febre":
        const filterFebre = await knex('forms').where('febre', Boolean(febre)).where('data', String(selectDate)).where('apto', false)
        response.json(filterFebre)
      break
      case "falta_ar":
        const filterFalta_ar= await knex('forms').where('falta_ar', Boolean(falta_ar)).where('data', String(selectDate)).where('apto', false)
        response.json(filterFalta_ar)
      break
      case "calafrio":
        const filterCalafrio= await knex('forms').where('calafrio', Boolean(calafrio)).where('data', String(selectDate)).where('apto', false)
        response.json(filterCalafrio)
      break
      case "garganta":
        const filterGarganta = await knex('forms').where('garganta', Boolean(garganta)).where('data', String(selectDate)).where('apto', false)
        response.json(filterGarganta)
      break
      case "cabeça":
        const filterCabeça = await knex('forms').where('cabeça', Boolean(cabeça)).where('data', String(selectDate)).where('apto', false)
        response.json(filterCabeça)
      break
      case "corpo":
        const filterCorpo = await knex('forms').where('corpo', Boolean(corpo)).where('data', String(selectDate)).where('apto', false)
        response.json(filterCorpo)
      break
      case "olfato":
        const filterOlfato = await knex('forms').where('olfato', Boolean(olfato)).where('data', String(selectDate)).where('apto', false)
        response.json(filterOlfato)
      break
      case "paladar":
        const filterPaladar = await knex('forms').where('paladar', Boolean(paladar)).where('data', String(selectDate)).where('apto', false)
        response.json(filterPaladar)
      break
      case "temperatura":
        const filterTemperatura = await knex('forms').where('temperatura', '>=', 37.2).where('data', String(selectDate)).where('apto', false)
        response.json(filterTemperatura)
      break
      default:
        const filterAll = await knex('forms').where('apto', false)
  ...
  async indexSelect(request: Request, response: Response) {
    ...
    switch (select) {
      case "data":
        const filterData = await knex('forms').where('data', String(selectDate)).whereNot('user_id', 135).orderBy('nome')
        response.json(filterData)
      break
      case "infectado":
      ...
      break
      case "visitante":
        const filterVisitante = await knex('forms').where('user_id', 135).where('data', String(selectDate))
